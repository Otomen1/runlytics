export const ACT_ICN={"Run":"🏃","Walk":"🚶","Hike":"⛰️","TrailRun":"🌳","VirtualRun":"💻"};
export const ACT_CLR={"Run":"var(--or)","Walk":"var(--gn)","Hike":"#8b5cf6","TrailRun":"#14b8a6","VirtualRun":"var(--bl)"};
export const IC={"rest":"var(--gn)","easy":"var(--or)","workout":"var(--rd)","long":"var(--bl)"};
export const IC_BG={"rest":"rgba(34,197,94,.08)","easy":"rgba(249,115,22,.06)","workout":"rgba(239,68,68,.08)","long":"rgba(59,130,246,.08)"};
export const IC_BD={"rest":"rgba(34,197,94,.18)","easy":"rgba(249,115,22,.15)","workout":"rgba(239,68,68,.18)","long":"rgba(59,130,246,.18)"};

export const TABS=[
  {id:"home",icon:"🏠",label:"Home"},
  {id:"stats",icon:"📊",label:"Stats"},
  {id:"hr",icon:"❤️",label:"HR Zones"},
  {id:"tasks",icon:"✅",label:"Habits"},
  {id:"awards",icon:"🏆",label:"Awards"},
];
